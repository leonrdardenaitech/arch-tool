...
// Mock API for Portfolio Preview
const MOCK_STATE = {
  agent_id: "ATLAS-01-PREVIEW",
  status: "IDLE",
  uptime: "04:22:18:09",
  last_sync: new Date().toISOString(),
  logs: [
    { timestamp: new Date().toISOString(), level: "INFO", message: "ATLAS CORE V2 Portfolio Preview Initialized." },
    { timestamp: new Date().toISOString(), level: "SUCCESS", message: "Bastion Host handshake secure." },
    { timestamp: new Date().toISOString(), level: "INFO", message: "Awaiting Director instructions..." }
  ],
  hitl_queue: [],
  documents: [
    { id: "1", name: "Darden_Array_Manifest.pdf", uploadedAt: new Date().toISOString() },
    { id: "2", name: "System_Recovery_Log.md", uploadedAt: new Date().toISOString() }
  ]
};

export default function AtlasCoreV2App() {
  ...
  const handleCommand = async (e) => {
    ...
    // Log User Request
    const newLogs = [...state.logs, { timestamp: new Date().toISOString(), level: 'INFO', message: `Processing User Request: ${currentPrompt}` }];
    ...
    setTimeout(() => {
      ...
      if (isSensitive) {
        const hitlLog = [...newLogs, { timestamp: new Date().toISOString(), level: 'WARN', message: 'Sensitive operation detected. HITL authorization required.' }];
        const newHitl = {
        ...
        setState(prev => ({ ...prev, logs: hitlLog, hitl_queue: [...prev.hitl_queue, newHitl] }));
      } else {
        const successLog = [...newLogs, { 
          timestamp: new Date().toISOString(), 
          level: 'SUCCESS', 
          message: `ATLAS CORE: Request analyzed. Protocol ${currentPrompt.toUpperCase()} initiated in sandbox environment.` 
        }];
        setState(prev => ({ ...prev, logs: successLog }));
      }
      setIsProcessing(false);
    }, 1500);
  };

  const approveHITL = (id, action) => {
    const updatedQueue = state.hitl_queue.filter(t => t.id !== id);
    const actionMsg = action === 'approve' ? 'Authorized' : 'Rejected';
    const status = action === 'approve' ? 'SUCCESS' : 'WARN';
    
    setState(prev => ({
      ...prev,
      hitl_queue: updatedQueue,
      logs: [...prev.logs, { timestamp: new Date().toISOString(), level: status, message: `HITL Task ${id} ${actionMsg}.` }]
    }));
  };

  return (
    <div className="h-full w-full bg-[#0A0A0A] text-zinc-300 font-sans selection:bg-emerald-500/30 selection:text-emerald-400 overflow-hidden flex flex-col">
      {/* Header / StatusBar */}
      <header className="border-b border-zinc-800 bg-zinc-900/50 backdrop-blur-md px-6 py-3 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-tighter text-white uppercase italic">Atlas_Core_v2</h1>
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4 text-[10px] font-mono tracking-wider uppercase text-zinc-500">
            ...
            <div className="flex flex-col items-end border-l border-zinc-800 pl-4">
              ...
              <span className="text-white">{state.uptime}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 grid grid-cols-12 gap-6 overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 overflow-hidden">
          {/* Main Interface */}
          <section className="bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden flex flex-col flex-1 shadow-2xl relative">
            ...
            {/* Terminal Logs (IPC) */}
            <div className="flex-1 p-6 font-mono text-sm overflow-y-auto space-y-4 scrollbar-thin scrollbar-thumb-zinc-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-5">
              ...
              <div className="space-y-2">
                ...
                <div className="mt-4 flex items-center justify-center gap-2 py-2 border border-dashed border-zinc-800 rounded hover:bg-zinc-800/50 transition-colors cursor-pointer group">
        ...
        <div className="col-span-12 lg:col-span-4 flex flex-col gap-6 overflow-hidden">
          {/* HITL Queue */}
          <section className="bg-zinc-900 border border-zinc-800 rounded-lg p-5 flex flex-col overflow-hidden">
      ...
      {/* Footer Branding */}
      <footer className="p-6 border-t border-zinc-800 flex flex-col items-center gap-2">
